...
void run_test(void)
{
    int ns[] = {50, 100, 300, 500, 700, 1000};
    double percents[] = {0.1, 0.2, 0.275, 0.3, 0.5, 0.7, 1.0};
    int n_count = sizeof(ns) / sizeof(ns[0]);
    int p_count = sizeof(percents) / sizeof(percents[0]);

    printf(
        "------------------------------------------------------------------------------------------------------------------------\n"
        "|  Кол-во   | %% от макс |  Матричное представление  |   Списочное представлени  | Сравнение памяти | Сравнение времени |\n"
        "|  вершин   |   Кол-во  |-------------------------------------------------------|    Матричное к   |    Списочное к    |\n"
        "|           |   ребер   | Память, байт | Время, сек | Память, байт | Время, сек |    Списочному    |    Матричному     |\n"
        "------------------------------------------------------------------------------------------------------------------------\n"
    );
    for (int i = 0; i < n_count; ++i)
    {
        ...
        for (int j = 0; j < p_count; ++j)
        {
            ...
            double k_time = (time1 - time2) / time1 * 100;
            double k_mem = (double)(mem1 - mem2) / mem1 * 100;
            // printf("(%ld - %ld) / %ld * 100 = %lf\n", mem1, mem2, mem1, k_mem);

            printf("| %9d | %9.1lf | %12ld | %10.5f | %12ld | %10.5f | %16.2lf | %17.2lf |\n", n, (percents[j] * 100), mem1, time1, mem2, time2, k_mem, k_time);
        }
        printf("------------------------------------------------------------------------------------------------------------------------\n");
    }
}

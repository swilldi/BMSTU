...
void print_null_node(bin_tree_node *node, FILE *dot_file);
void print_node(bin_tree_node *node, void *dot_file);
...
{
    bin_tree_node *node = (bin_tree_node*)tree;
    fprintf(dot_file, "digraph {\n");
    bin_tree_apple_pre(node, print_node, dot_file);
    fprintf(dot_file, "}\n");
}

void print_node(bin_tree_node *node, void *dot_file)
{
    bin_tree_node *left = bin_tree_left_child(node), *right = bin_tree_right_child(node);
    
    // Левый потомок
    if (left)
        fprintf(dot_file, "  "DOT_DEPENDENS_FMT, (char *)bin_tree_value(node), (char *)bin_tree_value(left));
    else
        print_null_node(node, dot_file);

    // Правый потомок
    if (right)
        fprintf(dot_file, "  "DOT_DEPENDENS_FMT, (char *)bin_tree_value(node), (char *)bin_tree_value(right));
    else
        print_null_node(node, dot_file);   
}
...
void print_null_node(bin_tree_node *node, FILE *dot_file)
{
    ...
    fprintf(dot_file, "  "NULL_DOT_DEPENDENS_FMT, (char *)bin_tree_value(node), null_count);

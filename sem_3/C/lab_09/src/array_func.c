...
void movie_copy(movie_t *movie, movie_t *source_movie)
{
    ...
    movie->title = strdup(source_movie->title);
    movie->director = strdup(source_movie->director);
    movie->year = source_movie->year;
}
...
void insert_sorted_by_field(movie_t *movie, movie_t *arr, size_t count, field sort_field)
{
    ...
    {
        i--;
        movie_copy(&arr[i + 1], &arr[i]);
    }
    movie_copy(&arr[i], movie);
}

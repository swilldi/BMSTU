...
void Controller::floor_destination_slot(floor_t floor)
{
    ...
    for (size_t i = 0; i < CABINS_COUNT; ++i)
    {
        ...
        {
            qInfo("[Контроллер] Запрос отклонён: вызов на этаж %d уже закреплён за лифтом %d",
                  floor, id + 1);
            return;
        }
    }
    ...
    qInfo("[Контроллер] Выбран лифт %d (вес=%.2f) для этажа %d",
    ...
    _floor_buttons[floor - 1]->activate_slot(); // UI ответ через сигнал кнопки
...
{
    qInfo("[Контроллер] Запрос из кабины %d на этаж %d", id + 1, floor);
    ...
    _cabin_buttons[id][floor - 1]->activate_slot(); // UI ответ через сигнал кнопки
...
void Controller::manage_move_slot(CabinID id)
{
    ...
    qInfo("[Лифт %d] Прошёл этаж %d → %d (направление=%s)",
...
void Controller::manage_cabin_slot(CabinID id)
{
    ...
    {
        qInfo("[Лифт %d] Едем вниз (текущий=%d, целевой=%d)",
              id + 1, _current_floor[id] + 1, next_floor + 1);
    ...
    {
        qInfo("[Лифт %d] Остановка на этаже %d", id + 1, _current_floor[id] + 1);
...
void Controller::reach_dst_floor_slot(CabinID id)
{
    ...
    qInfo("[Лифт %d] === Обработка задач на этаже %d ===", id + 1, floor);
    qInfo("[Лифт %d] Задач до удаления: %lu", id + 1, _task_manager.get_count_for_cabin(id));
    ...
    qInfo("[Лифт %d] Задач после удаления: %lu", id + 1, _task_manager.get_count_for_cabin(id));
    _task_manager.print_tasks();
...
floor_t Controller::get_next_visit_floor(CabinID id)
{
    ...
    if (tasks.empty())
    {
        qInfo("[Лифт %d] Задач нет", id + 1);
        return FLOOR_NOT_FOUND;
    }

    const floor_t floor = _current_floor[id] + 1;
    qInfo("[Лифт %d] Поиск следующего этажа (задач: %lu, текущий этаж: %d, направление: %s)",
          id + 1, tasks.size(), floor, dir_str(_current_directions[id]));

    if (_task_manager.has_for_cabin_and_floor(id, floor))
    {
        qInfo("[Лифт %d] Есть задача на текущем этаже %d → остановка", id + 1, floor);
    ...
    if (_current_directions[id] == IDLE)
    {
        ...
        qInfo("[Лифт %d] Стоит: предпочтительное=%s, задачи по ходу=%s, в обратном=%s",
        ...
        else if (has_tasks_in_opposite)
        {
            ...
            qInfo("[Лифт %d] Разворот: новое предпочтительное направление=%s",
                  id + 1, dir_str(_preferred_directions[id]));
            ...
            if (_preferred_directions[id] == UP)
            {
                for (floor_t new_floor = floor + 1; new_floor <= FLOOR_COUNT; ++new_floor)
                {
                    ...
                    {
                        qInfo("[Лифт %d] После разворота едем вверх к этажу %d", id + 1, new_floor);
                        return new_floor - 1;
                    }
                }
            }
            else
            {
                for (floor_t new_floor = floor - 1; new_floor >= 1; --new_floor)
                {
                    if (_task_manager.has_cabin_call(id, new_floor) || _task_manager.has_floor_call(id, new_floor))
                    {
                        qInfo("[Лифт %d] После разворота едем вниз к этажу %d", id + 1, new_floor);
                        return new_floor - 1;
                    }
                }
            }
        }
        qInfo("[Лифт %d] Нет задач ни в одном направлении", id + 1);
        return FLOOR_NOT_FOUND;
    }
    else
    {
    ...
    qInfo("[Лифт %d] Целевой этаж не найден", id + 1);
    return FLOOR_NOT_FOUND;
}
...
Direction Controller::get_next_direction(CabinID id)
{
    ...
    qInfo("[Лифт %d] Текущий этаж=%d, целевой=%d", id + 1, current_real_floor, target_real_floor);
    ...
    if (direction == UP)
    {
        ...
        for (floor_t floor = current_floor + 1; floor <= FLOOR_COUNT; ++floor)
        {
            if (_task_manager.has_cabin_call(id, floor) || _task_manager.has_floor_call(id, floor))
            {
                ...
                {
                    qInfo("[Лифт %d] Найден этаж %d по пути к конечному этажу %d",
                          id + 1, floor, furthest_up_call);
                    return floor - 1;
                }
                else if (furthest_up_call == FLOOR_NOT_FOUND)
                {
                    qInfo("[Лифт %d] Найден этаж %d по ходу вверх", id + 1, floor);
                    return floor - 1;
                }
            }
        }
        ...
        {
            qInfo("[Лифт %d] Едет к этажу %d для смены направления", id + 1, furthest_up_call);
    ...
    else if (direction == DOWN)
    {
        ...
        for (floor_t floor = current_floor - 1; floor >= 1; --floor)
        {
            if (_task_manager.has_cabin_call(id, floor) || _task_manager.has_floor_call(id, floor))
            {
                ...
                {
                    qInfo("[Лифт %d] Найден этаж %d по ходу вниз", id + 1, floor);
                    return floor - 1;
                }
            }
        }
        ...
        {
            qInfo("[Лифт %d] Едет к этажу %d для смены направления", id + 1, furthest_down_call);
